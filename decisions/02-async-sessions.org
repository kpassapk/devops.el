#+TITLE: Decision 2: Async execution in ob-shell sessions
#+STATUS: Implemented
#+DATE: 2026-07-30
...
(=ob-python= has had it since 9.6)
...
/Superseded by [[file:03-dynamic-targets.org][decision 3]]:/ once a target can
change between runs, a session named by tag alone reuses a shell on the wrong
host. The default now folds in the target, =devops:TAG TARGET=. The variable
stays for anyone who wants the short form back.
...
The same contract holds outside tangling whenever one block's output feeds
another. A =:var NAME=name= on a src block, a =#+call: say-hi(NAME=name)=
argument, and an executing noweb reference all resolve through
=org-babel-ref-resolve=, which executes the named block and hands back its
return value. Under async that value is the placeholder, so the caller binds a
UUID and prints it (issue 14). Worse, when both blocks report in one chunk the
comint async filter finds the reference's UUID inside the caller's freshly
inserted result and overwrites the caller's result with the reference's output.
So =org-babel-ref-resolve= is advised to run under =devops-with-sync=: the
block the user invoked stays async, but everything executed to resolve its
inputs is synchronous.
